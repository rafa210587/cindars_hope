...
{
    /// <summary>
    /// SPEC 14A-FIX5 – Validates that enemy spawn profiles, packs and locks cover all cave level bands.
    /// Run via: CindarsHope/Validate/Enemy Cave Spawn Coverage
    /// </summary>
    public static class ValidateEnemyCaveSpawnCoverage
    {
        private static readonly int[] SampleLevels = { 1, 10, 15, 25, 30, 40, 45, 55, 60, 70, 71, 75, 85, 86, 90, 99 };
        ...
        public static void Validate()
        {
            ...
            if (failures == 0)
                Debug.Log("[CoverageValidator] All sampled levels PASSED coverage check.");
            else
                Debug.LogError($"[CoverageValidator] {failures}/{SampleLevels.Length} sampled levels FAILED. See warnings above.");
        }
            ...
            EnemyFactionLockSO[] locks)
        {
            ...
            var summary = errors.Count == 0
                ...
                : string.Join("; ", errors) + $" | biome={biomeTag}, profiles={eligibleProfiles.Count}, packs={eligiblePacks.Count}";
