...
namespace CindarsHope.Editor.Validation
{
    ...
    internal sealed class FarmDoorAndWaterProbe : IDisposable
    {
        [Serializable] internal sealed class Sample
        {
            ...
            public Vector2 actualMoveInput, actualFacing;
            public Vector2 physicalVelocity;
        ...
        [Serializable] internal sealed class Evidence
        {
            ...
            public string method = "Initial framing at the earlier physically validated house selection position. Thereafter actual Rigidbody2D.MovePosition in FixedUpdate with PlayerController temporarily disabled. PlayerWalkAnimator observes actual physical velocity. Live selector invokes only HouseDoorInteractable. No forced sprite, facing, alpha, door state, injected input, portal, save or load. Water reads actual Tilemap.GetAnimationFrame after rendering. This is controlled-body traversal, not a movement-input test or human playthrough.";
            ...
            try
            {
                ...
                switch (phase)
                {
                    ...
                    case Phase.Water:
                        ...
                        var movesUp = false;
                        var movesDown = false;
                        for (var sampleIndex = 1; sampleIndex < Result.samples.Count; sampleIndex++)
                        {
                            var current = Result.samples[sampleIndex];
                            var previous = Result.samples[sampleIndex - 1];
                            if (!current.phase.StartsWith("traverse_", StringComparison.Ordinal)) continue;
                            var verticalTravel = current.playerPosition.y - previous.playerPosition.y;
                            movesUp |= verticalTravel > 0.025f && current.playerSprite.StartsWith("walk_up_", StringComparison.Ordinal);
                            movesDown |= verticalTravel < -0.025f && current.playerSprite.StartsWith("walk_down_", StringComparison.Ordinal);
                        }
                        Require(movesUp && movesDown,
                            "Physical traversal did not expose the runtime's walking frames in both entry and exit directions.");
                        Result.checks.Add("Actual moving-body samples show runtime walk_up and walk_down sprites; no sprite or facing was injected.");
        ...
        private Sample Capture(string id, bool captureWater = false)
        {
            ...
            sample.actualMoveInput = player.MoveInput;
            sample.actualFacing = player.LastFacingDirection;
            sample.physicalVelocity = body.linearVelocity;
            sample.playerSprite = player.GetComponent<SpriteRenderer>().sprite.name;
