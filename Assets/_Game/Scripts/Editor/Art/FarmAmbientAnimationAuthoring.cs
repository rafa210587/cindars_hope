...
namespace CindarsHope.Editor.Art
{
    ...
    public static class FarmAmbientAnimationAuthoring
    {
        ...
        public static ValidationReport Apply(Scene scene)
        {
            ...
            var fountain = fountainRoot.transform.Find("FountainDepthGroup/Visual")?.GetComponent<SpriteRenderer>();
            ...
            var boat = Components<SpriteRenderer>(scene).Single(r => r.name == "Visual_MooredBoat");
            BindExisting(boat, "boat_v18", 4, 1250);
        ...
        {
            foreach (var id in new[] { "fountain_v15", "cascade_v15", "fish_v15", "boat_v18" })
            {
        ...
        public static ValidationReport ValidateGenerated()
        {
            ...
            foreach (var id in new[] { "fountain_v15", "cascade_v15", "fish_v15", "boat_v18" })
            {
                ...
                var boat = id == "boat_v18";
                var count = fish ? 8 : boat ? 4 : 5;
                var period = fish ? FishPeriod : boat ? 5f : 0.7f;
