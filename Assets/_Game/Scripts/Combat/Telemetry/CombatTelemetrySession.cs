...
{
    /// <summary>
    /// fable_59 — estado AGREGADO de telemetria de combate, PURO (sem Unity), por nível da caverna.
    /// Toda a matemática (TTK, % de dano recebido, somatórios, contadores) vive aqui para ser testável
    /// em EditMode com eventos sintéticos. O <see cref="CombatTelemetryService"/> só repassa os dados
    /// dos eventos para os métodos Record* desta classe e injeta o relógio (timestamps em segundos),
    /// exatamente como CombatWindowRules evita depender de Time.time em teste.
    ///
    /// TTK por enemyId: o cronômetro abre no PRIMEIRO dano sofrido pela criatura
    /// (<see cref="RecordEnemyDamaged"/>) e fecha na morte (<see cref="RecordEnemyKilled"/>):
    /// TTK = tMorte - tPrimeiroDano. Mortes sem dano registrado são DESCARTADAS (gap logado),
    /// nunca contam TTK=0.
    /// </summary>
    public sealed class CombatTelemetrySession
    {
        ...
        // enemyId → instante (s) do primeiro dano sofrido (cronômetro aberto). Reaberto a cada kill.
        ...
        // último HP máximo conhecido (para % de dano recebido); 0 = desconhecido → % não computável.
        private int _lastKnownMaxHp;

        private int _damageDealtTotal;
        private int _damageTakenTotal;
        private double _damageTakenPctSum; // soma dos % por hit, para a média do trecho
        ...
        // último valor lido (para derivar deltas negativos = gasto) — sentinela -1 = não lido ainda.
        ...
        /// <summary>Marca o início do trecho (entrada no nível). Limpa deltas de stamina/mana.</summary>
        public void BeginLevel(int caveLevel, int band, string runId, float nowSeconds)
        {
            ...
            _hasStart = true;
            // baseline de stamina/mana só conta a partir do próximo evento (evita delta gigante inicial).
            _lastStamina = -1;
            _lastMana = -1;
        }
        ...
        /// <summary>Média do % de HP perdido por hit no trecho (0 se nenhum hit com MaxHP conhecido).</summary>
        ...
        // ── Coletores (chamados pelo service a partir dos eventos) ───────────────────────────────

        /// <summary>Primeiro dano sofrido por uma criatura abre/atualiza o cronômetro de TTK.</summary>
        public void RecordEnemyDamaged(string enemyId, int amount, string damageType, float nowSeconds)
        {
            ...
            // EnemyDamagedEvent é dano DADO ao inimigo → conta como dano dado e por tipo.
            if (amount > 0)
            {
        ...
        /// <summary>
        /// Dano DADO genérico (DamageAppliedEvent) — soma ao total apenas se NÃO houver enemyId já
        /// contabilizado por RecordEnemyDamaged. Como DamageAppliedEvent e EnemyDamagedEvent podem
        /// representar o mesmo golpe, esta entrada é usada só para o total quando o chamador indicar
        /// que é uma fonte distinta. Mantida separada para evitar dupla contagem por padrão.
        /// </summary>
        ...
        /// <summary>Morte da criatura fecha o cronômetro: TTK = morte - primeiro dano (se houver).</summary>
        public void RecordEnemyKilled(string enemyId, float nowSeconds)
        {
            ...
            if (_firstDamageTimeByEnemy.TryGetValue(enemyId, out var firstDamage))
            {
                ...
                agg.AddTtk(ttk);
                _firstDamageTimeByEnemy.Remove(enemyId); // próxima instância reabre o cronômetro
            }
            else
            {
        ...
        /// <summary>Dano recebido pelo player: total, por fonte e % do HP máx no momento do hit.</summary>
        ...
        /// <summary>Atualiza o HP máx conhecido (de HPChangedEvent) para o cálculo de % de dano.</summary>
        ...
        // ── Consultas para teste/relatório ───────────────────────────────────────────────────────

        /// <summary>TTK médio (s) de um enemyId; 0 se não há kills com dano registrado.</summary>
        ...
        /// <summary>Constrói o DTO serializável com todas as avaliações §6-§7 preenchidas.</summary>
        public CombatTelemetryReport BuildReport(string generatedAtUtc, IList<string> extraGaps)
        {
            ...
            {
                report.Gaps.Add("damage taken recorded but MaxHP unknown at hit time → % targets not evaluated");
            }
