...
OUT = r'C:\Users\Rafa\Downloads\Sprites\Fazendeiro\walkright'

F = [Image.open(f).convert('RGBA') for f in sorted(glob.glob(os.path.join(SP, 'faz_proc', 'Diagonal_direita', 'sl', 'f*.png')))]
seq = [0, 2, 3, 4, 0, 1, 0]   # perfil, sem espelhar
...
for k, fr in enumerate(frames, 1):
    ...
    cell.save(os.path.join(OUT, 'walk_right_%02d.png' % k))
    cells.append(cell)
...
gi[0].save(os.path.join(OUT, 'walk_right.gif'), save_all=True, append_images=gi[1:], duration=170, loop=0, disposal=2)
print('cell', cw, ch, '| 7 frames + walk_right.gif ->', OUT)
