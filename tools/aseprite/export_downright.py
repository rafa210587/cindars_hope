...
F = [Image.open(f).convert('RGBA') for f in sorted(glob.glob(os.path.join(SP, 'faz_proc', 'Diagonal_direita_baixo', 'sl', 'f*.png')))]
seq = [0, 3, 1, 2, 0, 4, 0]   # sem espelhar (down-direita)
...
# down-direita: cada frame num cell uniforme bottom-center
cells_r = []
for fr in frames:
    cell = Image.new('RGBA', (cw, ch), (0,0,0,0))
    cell.alpha_composite(fr, ((cw-fr.width)//2, ch-fr.height))
    cells_r.append(cell)
export('walkdownright', 'walk_downright', cells_r)

# down-esquerda = flip de cada cell
cells_l = [c.transpose(Image.FLIP_LEFT_RIGHT) for c in cells_r]
export('walkdownleft', 'walk_downleft', cells_l)
