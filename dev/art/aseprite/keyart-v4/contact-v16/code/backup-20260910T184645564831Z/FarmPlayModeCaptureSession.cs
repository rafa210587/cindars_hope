    ...
    internal static class FarmPlayModeCaptureSession
    {
        ...
        private static void Tick()
        {
            ...
            try
            {
                ...
                if ((DateTime.UtcNow - DateTime.Parse(clock).ToUniversalTime()).TotalSeconds > ((session.motionRequested || session.ambientOnlyRequested) ? 180 : TimeoutSeconds))
                {
                ...
                if ((session.stage != "capturing" && session.stage != "interactions" && session.stage != "motion" && session.stage != "ambient") || !EditorApplication.isPlaying) return;
