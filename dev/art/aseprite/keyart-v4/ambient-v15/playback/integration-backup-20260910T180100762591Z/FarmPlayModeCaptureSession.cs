    ...
    internal static class FarmPlayModeCaptureSession
    {
        ...
        private static void Tick()
        {
            ...
            try
            {
                ...
                if ((DateTime.UtcNow - DateTime.Parse(clock).ToUniversalTime()).TotalSeconds > (session.motionRequested ? 180 : TimeoutSeconds))
                {
                ...
                if ((session.stage != "capturing" && session.stage != "interactions" && session.stage != "motion") || !EditorApplication.isPlaying) return;
        ...
        private static void Complete()
        {
            ...
            finally
            {
                ...
                File.WriteAllText(Path.Combine(OutputDirectory, "capture-metadata.json"), JsonUtility.ToJson(session, true));
                if (pass) Debug.Log("Farm gameplay capture: PASS (" + ViewIds.Length + "/" + ViewIds.Length + ")");
