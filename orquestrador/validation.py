...
def normalize_spec_id(spec_id) -> Optional[str]:
    """
    Normalize spec ID to format SPEC_012.
    Accepts: 12, "12", "SPEC_12", "SPEC_012"
    """
    if spec_id is None:
        return None

    # If int, convert to string first
    if isinstance(spec_id, int):
        return f"SPEC_{spec_id:03d}"
    ...
    # If just digits
    if s.isdigit():
        return f"SPEC_{int(s):03d}"

    # If SPEC_XX format
    m = re.match(r"SPEC_(\d+)$", s, re.IGNORECASE)
    if m:
        return f"SPEC_{int(m.group(1)):03d}"
...
def run_unity_scan(repo_root: Path, log_path: Path, unity_compile_log_path: Optional[Path] = None) -> tuple[str, str]:
    """
    Run Unity log scan via PowerShell script.
    Uses the log generated by RunUnityCompileValidation.ps1
    Returns (status: str, output: str)
    """
    ...
    try:
        # If unity_compile_log_path provided, use it; otherwise use default
        if unity_compile_log_path:
            log_file = str(unity_compile_log_path)
        else:
            log_file = str(repo_root / "Logs" / "unity-compile-validation.log")
...
def run_repo_checks_spec(repo_root: Path, spec_id: str, log_path: Path) -> tuple[str, list[str]]:
    ...
    Accepts spec_id in formats: 12, "12", "SPEC_12", "SPEC_012"
    Returns (status: str, found_issues: list[str])
    """
    # Normalize spec_id
    normalized_id = normalize_spec_id(spec_id)
    if not normalized_id:
        output = f"Invalid spec_id: {spec_id}"
        log_path.write_text(output, encoding="utf-8")
        return "PASS", []

    # Spec-specific checks - use normalized format as key
    spec_checks = {
        "SPEC_012": [
    ...
    if normalized_id not in spec_checks:
        output = f"No spec-specific checks for {normalized_id}"
        log_path.write_text(output, encoding="utf-8")
        return "PASS", []

    checks = spec_checks[normalized_id]
    issues = []
    outputs = []
...
def run_all_validations(
    ...
    if config.get("require_unity_compile", True):
        ...
        unity_log_path = log_dir / "unity_compile_validation.log"
    ...
    if unity_status == "PASS":
        run_unity_scan(repo_root, log_dir / "unity_scan.log", unity_log_path)
    ...
    if config.get("require_repo_checks", True):
        ...
        if spec_id:
            run_repo_checks_spec(repo_root, spec_id, log_dir / f"repo_checks_{normalize_spec_id(spec_id) or spec_id}.log")
