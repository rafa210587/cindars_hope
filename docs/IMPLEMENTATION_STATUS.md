...
| Architecture Reorganization Combat Services & Item Contracts (SpecKit SPEC_04-08) | Implementado em codigo - validacao Unity/Play Mode pendente | SPEC_04 (wave 1 legacy audit); SPEC_05 (CombatActionContext+EquippedItemResolver+CooldownHelper); SPEC_05B (RefreshItemResolver fix); SPEC_06 (ProjectileSpawnService+ProjectileSpawnRequest/Result); SPEC_07 (BowArrowAttackService+SpellCastService+AttackResult); SPEC_07B (RebindStaminaManager + bow bloqueio pós-resolution); SPEC_08 (ItemUseKind enum + ItemUseContractResolver + ItemDataSO 4 campos + CombatDatabaseValidator validacoes). Builds C# 0E/0W runtime, 0E/2W editor (pre-existentes) em 2026-06-01. Comportamento gameplay 100% preservado. Backward compat total via fallback Category. Unity compile validation e Play Mode humano pendentes. |
