...
|---|---|---|
| `Inventory\|Save` | InventoryManager→Save | Mover `InventorySaveData`/`InventorySlotSaveData`/`InventoryItemSaveData` p/ `CindarsHope.Inventory` (ou Foundation). |
| `Save\|UI` | Save→UI (layering errado) | Mover `HotbarState`+`HotbarSaveData` (puros) p/ Save/Foundation; inverter posse do `OnboardingHintsSectionProvider` p/ o lado UI registrar no registry. |
