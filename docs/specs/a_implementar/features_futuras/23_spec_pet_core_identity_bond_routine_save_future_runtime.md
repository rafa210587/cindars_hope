# SPEC — Pet Core Identity Bond Routine Save Future Runtime

> **Spec ID:** `23_spec_pet_core_identity_bond_routine_save_future_runtime`  
> **Status:** A implementar / Future mapped / HOLD until explicit pet-scope approval  
> **Wave:** WAVE 23 — Pets Advanced Future  
> **Priority:** P1  
> **Type:** Runtime / Future / HOLD / Pet Core  
> **Domain:** Pet Identity / Bond / Mood / Energy / Routine / Save Contracts  
> **Parallelizable:** NO  
> **Parallel group:** WAVE_23_PETS_ADVANCED_FUTURE  
> **Can run with:** N/A  
> **Must not run with:** qualquer spec que altere companion runtime, social relationship runtime, save schema migration, UI HUD, farm/cave behavior or scene/prefab assets.  
> **Repo lock scope:** `Assets/_Game/Scripts/Pets/**`, `Assets/_Game/Tests/EditMode/Pets/**`, `docs/validation/23_spec_pet_core_identity_bond_routine_save_future_runtime_execution_report.md`  
> **Depends on:**  
...
> **Blocks:**  
  ...
  - pet cave alerts;
  - pet HUD/data assets.
> **Scope:** definir contratos futuros de identidade, vínculo, humor, energia, rotina e save de pet, mantendo HOLD até aprovação explícita.  
> **Out of scope:** runtime visual, HUD, farm/cave behavior, companion integration, data assets Unity, save migration.
...
Pets estão deferidos, mas o direction define que, quando voltarem ao escopo, o baseline deve suportar cachorro e gato; vínculo, humor, energia, trust flags e places conhecidos; rotina diária simples; presença em casa/fazenda/caverna; save por IDs e valores simples.
...
```text
pet vira companion;
pet não tem energia/cooldown;
pet some sem feedback;
pet ganha Breath/Fôlego por herdar player resources;
save serializa GameObject/Transform;
alimentação vira microgerenciamento pesado;
falta de comida mata pet no baseline;
pet vira requisito de progressão.
```
...
Pet não é companion.
Baseline suporta cachorro e gato.
Escolha cachorro/gato é majoritariamente afetiva/estética.
Pet não morre por falta de alimentação no baseline.
...
```text
Como jogador, quero ter um pet com nome, espécie, vínculo, humor e rotina.
Como save/load, quero preservar pet por IDs e valores simples.
Como rotina, quero que pet seja encontrável e previsível.
Como balance, quero pet sem Breath/Fôlego, sem companion slot e sem progressão obrigatória.
Como UI futura, quero projetar estado sem ser fonte de verdade.
```
...
FerretFuture
OwlFuture
LizardMiniDrakeFuture
RabbitFuture
GoatFuture
MagicalFontePetFuture
DebugOnly
```
...
LastFedDay
LastInteractionDay
LastGiftedItemId optional
HomeAnchorId optional
...
```text
Pet starts day near bed/area if owned.
Pet can explore farm if mood/energy allow.
Pet can follow player only if called and allowed.
Pet rests when energy low.
Pet returns to bed/area at night.
Pet does not vanish randomly; unavailable states need reason.
```
...
```text
Do not serialize GameObject.
Do not serialize Transform.
Do not serialize ScriptableObject reference.
Do not serialize Unity reference.
Do not serialize Breath/Fôlego.
Use stable IDs and simple values.
If pet home anchor missing, fallback to safe home position.
If active pet invalid, clear activePetId and keep owned pet data.
```
...
```text
Pet core contracts exist.
Dog/Cat baseline supported.
Bond/mood/energy/presence separated.
Routine states predictable.
Save record uses IDs/simple values.
No Breath/Fôlego.
Tests cover create dog/cat, bond increase, mood/energy reduce, low energy unavailable, routine fallback, save/load no Unity refs and invalid active pet fallback.
```
...
```text
Assets/_Game/Scripts/Pets/Core/PetSpecies.cs
Assets/_Game/Scripts/Pets/Core/PetCoreState.cs
Assets/_Game/Scripts/Pets/Core/PetBondLevel.cs
Assets/_Game/Scripts/Pets/Core/PetMood.cs
Assets/_Game/Scripts/Pets/Core/PetEnergyState.cs
Assets/_Game/Scripts/Pets/Core/PetPresenceState.cs
Assets/_Game/Scripts/Pets/Core/PetRoutineState.cs
Assets/_Game/Scripts/Pets/Save/PetSaveRecord.cs
Assets/_Game/Scripts/Pets/Core/PetCoreValidator.cs
Assets/_Game/Tests/EditMode/Pets/PetCoreIdentityBondRoutineSaveTests.cs
```
...
docs/validation/23_spec_pet_core_identity_bond_routine_save_future_runtime_execution_report.md
```
...
- [ ] T002 — Ler fontes.
- [ ] T003 — Auditar pet/companion/save/player systems.
- [ ] T004 — Consolidar core contracts.
...
- Pets estão deferidos e não entram no escopo atual até decisão explícita.
- Baseline futuro deve suportar cachorro e gato.
- Pet tem vínculo, humor, energia, trust flags e places conhecidos.
- Pet tem rotina diária: casa/área, seguir jogador, descansar, dormir, explorar fazenda e cave active.
- Pet não morre por falta de alimentação no baseline; falta de cuidado reduz humor/energia/disponibilidade.
- Save/load futuro deve preservar IDs e valores simples e não serializar Unity references.
...
| Estado real do repo | Sistemas existentes relacionados a pet core identity/bond/routine/save foram auditados antes de criar novos? | Comandos `rg` e achados. | PARTIAL |
...
| Report | Execution report criado? | `docs/validation/23_spec_pet_core_identity_bond_routine_save_future_runtime_execution_report.md`. | PARTIAL |
...
```bash
rg -n "PetCore|PetBond|PetMood|PetEnergy|PetPresence|PetRoutine|PetSave|Dog|Cat|Folego|Breath" Assets/_Game/Scripts docs/design docs/specs
...
When o fluxo de pet core identity/bond/routine/save roda
...
- Pet treated as companion.
- Pet gets Breath/Folego.
- Pet dies from hunger baseline.
- Save serializes Unity references.
- Pet disappears randomly.
- Dog/cat choice becomes power optimal.
- Pet becomes progress requirement.
...
```md
# Execution Report — Pet Core Identity Bond Routine Save Future Runtime
...
- Changed deterministic logic: YES if executed; core/routine/save validation is deterministic.
- Requires EditMode tests: YES if executed for create/bond/mood/energy/routine/save/no-refs tests.
- Requires PlayMode automated or final human scenario: YES, DEFERRED for pet visual/routine validation.
- Requires regression test: YES if fixing existing pet core bug; otherwise NO.
...
Execution report criado em docs/validation/23_spec_pet_core_identity_bond_routine_save_future_runtime_execution_report.md.
