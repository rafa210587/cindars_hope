...
| Escopo | A execução ficou dentro de farm order / quest adapter? | Arquivos alterados e justificativa. | PARTIAL |
...
| Report | Execution report foi criado? | `docs/validation/03_spec_quest_farm_orders_adapter_runtime_execution_report.md`. | PARTIAL |
...
```bash
rg -n "FarmOrder|Order|Shipping|SellPoint|Quest|Objective|Reward|Inventory|Calendar|Deadline" Assets/_Game/Scripts docs/design docs/specs
...
```text
Given o sistema base relacionado a farm order / quest adapter existe ou foi criado de forma mínima
...
- Criar FarmOrderState paralelo a QuestState.
- Aplicar reward fora do pipeline idempotente.
- Perder prazo/deadline em save/load.
- Sell/shipping disparar completion sem quest trigger.
- Inventário/item entregue sem validação.
- UI/board final entrar no escopo indevidamente.
- Economia/balance ser alterada fora do escopo.
- Order repetível sem repeat policy.
