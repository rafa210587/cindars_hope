...
|---|---|---|---|---|
| 00.02 | [00_spec_wave_execution_protocol.md](a_implementar/00_spec_wave_execution_protocol.md) | A implementar - governanca | 00.01 roadmap master | Cria protocolo canônico de execução por waves, paralelização, locks, batch size, status e validação humana final deferida. |
| 00.05 | [00_spec_validation_matrix_master.md](a_implementar/00_spec_validation_matrix_master.md) | A implementar - validacao/governanca | 00.01 roadmap master + Testing Quality Gate | Cria matriz master de validação por tipo de mudança, evidência mínima, status caps e relação com validação humana final. |
