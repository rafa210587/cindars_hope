...
# SPEC 09 — Hunger, Stamina, Status e Time Balance
...
git checkout dev
git pull
git checkout -b feature/spec-09-hunger-stamina-status-time
```
...
Depende de 01-05. Bloqueia 10, 11, 12, 14 e 17.

## Objetivo desta execução

Fechar integração visual/runtime de Hunger/Stamina/Status/GameTime e validar fluxo fome -> stamina regen -> morte/respawn.
...
- Canvas UI final consolidada ou reclassificação clara para SPEC 17.
- Status effects com visuals: ícones, duração/stacking mínimo.
- Play Mode: fome reduz regen, fome zero drena/danifica conforme regra, morte/respawn não quebra managers.
- Confirmar wiring em GameBootstrap e scene installers.
- Confirmar save/load de GameTime, fome, stamina e status effects.

## Áreas permitidas para alteração

- `Assets/_Game/Scripts/Player/**`
- `Assets/_Game/Scripts/Core/Time/**`
...
- GameTimeManager inicializa e desliga corretamente.
- HUD recebe eventos e mostra valores coerentes.
- Status effects expiram/stackam conforme spec.
- Save/load preserva estado sem refs Unity.
...
```bash
git add <arquivos>
git commit -m "feat: finalizar spec 09 - hunger stamina status time"
```
