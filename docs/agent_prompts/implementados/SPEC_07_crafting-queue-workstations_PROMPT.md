...
# SPEC 07 — Crafting Queue, Workstations, Recipes e UI
...
git checkout dev
git pull
git checkout -b feature/spec-07-crafting-queue-workstations
```
...
Depende de 06. Bloqueia 10 e 17.

## Objetivo desta execução

Finalizar crafting com estações físicas, fila com tempo, cancelamento seguro, coleta, save/load e integração com inventory/economia.
...
- Reconciliar código real antes de implementar, pois registry pode estar desatualizado.
- Workstations físicas na fazenda: Workbench, Forge, CookingStation ou equivalentes aprovados pela spec.
- Job queue com `CraftTimeSeconds`.
- Cancelamento seguro devolvendo 100% dos ingredientes se houver espaço; se não houver, tratar rollback/feedback.
- Coleta de output.
- Modal de crafting integrado à modal stack.
- Save/load de job em andamento.
- Craft de bolso quando `RequiredStationType = None`.
- Test kit de recursos/recipes criado via Editor script idempotente.
- UI Canvas funcional mínima; polish final pode ficar para SPEC 17 se documentado.
...
- Craft inicia job e desconta ingredientes uma única vez.
- Tempo progride corretamente respeitando pause/modal quando aplicável.
- Cancelamento devolve ingredientes ou falha sem perda silenciosa.
- Output é coletável e persiste após save/load.
- Workstations existem em cena de forma reprodutível.
...
```bash
git add <arquivos>
git commit -m "feat: finalizar spec 07 - crafting queue workstations"
```
