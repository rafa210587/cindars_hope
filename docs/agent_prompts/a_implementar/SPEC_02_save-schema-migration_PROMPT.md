...
# SPEC 02 — Save Schema Migration v2/v3
...
git checkout dev
git pull
git checkout -b feature/spec-02-save-schema-migration
```
...
Depende da spec 01 para validação e serve de base para 03-17.

## Objetivo desta execução

Fechar save/migration para Mana, active skill slots e cave run snapshots, garantindo roundtrip save/load/save sem perda.
...
- ManaManager integrado em SaveData capture/apply.
- SkillActionSO active slot assignment salvo por `SkillActionId`, nunca por referência Unity.
- CaveRunManager snapshot/run state salvo por DTOs simples.
- Migration segura para saves legados.
- Teste roundtrip save -> load -> save com equivalência semântica.
- Documentar risco residual se Play Mode não puder rodar.
...
- SaveData atual inclui Mana e active slots por IDs.
- Cave run state/snapshots persistem sem Unity refs.
- Migration inicializa campos novos sem quebrar saves antigos.
- Roundtrip validado ou `NOT RUN` registrado com motivo.
...
```text
PLAY MODE TEST: SPEC 02 — Save Schema Migration v2/v3
Scene used:
Steps executed:
Expected result:
Observed result:
...
```bash
git add <arquivos>
git commit -m "feat: finalizar spec 02 - save schema migration"
```
