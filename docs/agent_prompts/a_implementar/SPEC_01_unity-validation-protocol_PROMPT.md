...
git checkout dev
git pull
git checkout -b feature/spec-01-unity-validation-protocol
```
...
Nenhuma dependência runtime; esta spec desbloqueia todas as demais validações.

## Objetivo desta execução

Fechar o protocolo de validação Unity para que specs runtime possam ser validadas sem depender de validação humana intermediária.
...
- Play Mode validation automatizado ou, se não for viável, runner/checklist padronizado acionável por comando.
- MissingScriptScanner.
- SceneReferenceValidator.
- DataIdValidator.
- Validação de prefabs/cenas contra Missing Script e referências quebradas.
- Documentar comandos, saída esperada e fallback NOT RUN.
...
- `RunUnityCompileValidation.ps1` e `ScanUnityLogs.ps1` continuam funcionando.
- Existe validação adicional para Missing Scripts, referências de cena/prefab e IDs de dados.
- Existe forma clara de registrar Play Mode validation sem depender do humano agora.
- Docs/status/registry ficam reconciliados.
...
```text
PLAY MODE TEST: SPEC 01 — Unity Compile Validation Protocol
Scene used:
Steps executed:
Expected result:
Observed result:
...
```bash
git add <arquivos>
git commit -m "feat: finalizar spec 01 - unity validation protocol"
```
