...
git checkout dev
git pull
git checkout -b feature/spec-04-farm-planting-ui-stamina
```
...
```text
docs/specs/implementados/spec_farm_004_irrigacao_solo_planting_ui.md
```

## Dependências diretas

Depende de 01-03. Bloqueia 05, 09 e 17.

## Objetivo desta execução

Fechar plantio/solo/irrigação com custo de stamina e validação funcional de plantar-crescer-colher.
...
- UI Canvas funcional para plantio/solo, ou reclassificar explicitamente parte visual final para SPEC 17.
- Play Mode completo: plantar, irrigar, avançar dia, colher.
- Stamina/custo de ações ligado ao StaminaManager.
- Feedback de ação bloqueada por falta de stamina/item/estado inválido.
- Save/load dos estados de solo/água/planta validado.
...
- Ações de farm gastam stamina via contrato único.
- Planting UI não conflita com modal stack.
- Estados de plot persistem.
- Play Mode checklist registrado.
...
```bash
git add <arquivos>
git commit -m "feat: finalizar spec 04 - farm planting ui stamina"
```
