...
git checkout dev
git pull
git checkout -b feature/spec-15-cave-entry-death-anya-corpse
```
...
```text
docs/specs/a_implementar/spec_cave_entry_death_anya_corpse_recovery.md
```

## Dependências diretas

Depende de 14. Bloqueia 16 e 17.
...
- Fonte de Anya como NPC/location definida e interagível.
- Morte cria corpse recuperável.
- Perda de inventory/equipment/gold para corpse conforme regra.
- XP volta para início do nível.
- Apenas último corpse ativo; morrer novamente substitui o anterior.
- Corpse persistence em save/load.
- Respawn com HP/Stamina/Mana cheios.
- Hunger NÃO é restaurada.
- Death UI screen mínimo.
- Corpse recovery modal mínimo.
- Redistribuição/respawn de inimigos ao morrer sem quebrar stable run.
- Play Mode: morte -> corpse -> recovery -> respawn -> save/load.
...
- Morte não destrói save/run state indevidamente.
- Corpse contém perdas e é recuperável.
- Nova morte substitui corpse anterior.
- Anya/Fonte tem interação funcional.
- Respawn preserva hunger conforme regra.
...
```text
PLAY MODE TEST: SPEC 15 — Cave Entry, Death, Anya e Corpse Recovery
Scene used:
Steps executed:
Expected result:
Observed result:
...
```bash
git add <arquivos>
git commit -m "feat: finalizar spec 15 - cave entry death anya corpse"
```
