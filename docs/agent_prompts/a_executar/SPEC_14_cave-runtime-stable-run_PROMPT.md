...
# SPEC 14 — Cave Runtime, Generation, Checkpoints e Boss Gates
...
git checkout dev
git pull
git checkout -b feature/spec-14-cave-runtime-stable-run
```
...
Depende de 13. Bloqueia 15 e 17.

## Objetivo desta execução

Finalizar runtime da caverna com modelo de 100 níveis, stable run guarantee, snapshots, checkpoints, boss gates, resource nodes e save/load completo.
...
- 100-level macro model consolidado.
- Stable run guarantee: nível já visitado na mesma CaveRunSeed recarrega snapshot, não rerolla.
- Snapshot reload garantido.
- Checkpoint portal UI/side menu mínimo.
- Boss gate progression real.
- Enemy spawn plan estável por run.
- Respawn de inimigos comuns sem mudar plano original.
- Boss item drops únicos sem duplicação.
- Fishing spots procedurais na cave.
- Resource nodes procedurais persistentes.
- Camera bounds validation.
- Save/load completo de run state.
- Play Mode: traversal, checkpoint, boss gate, return/reload, respawn.
...
- Visitar nível N, sair e voltar preserva layout/inimigos/resources conforme snapshot.
- Boss gates bloqueiam/desbloqueiam corretamente.
- Checkpoints persistem.
- Save/load restaura run state.
- Camera bounds e confinement não deixam player sair do mapa.
...
```text
PLAY MODE TEST: SPEC 14 — Cave Runtime, Generation, Checkpoints e Boss Gates
Scene used:
Steps executed:
Expected result:
Observed result:
...
```bash
git add <arquivos>
git commit -m "feat: finalizar spec 14 - cave runtime stable run"
```
