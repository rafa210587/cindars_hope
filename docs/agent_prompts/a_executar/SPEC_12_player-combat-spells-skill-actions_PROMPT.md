...
# SPEC 12 — Player Combat, Weapons, Spells e Skill Actions
...
git checkout dev
git pull
git checkout -b feature/spec-12-player-combat-spells-skill-actions
```
...
Depende de 09-11. Bloqueia 13, 14, 16 e 17.

## Objetivo desta execução

Finalizar combate do jogador: ManaManager integrado, prioridade da tecla E para interactables, ataques, arco/projétil, spells, active slots R/T/Y/G, assets mínimos e save/load.
...
- ManaManager integrado em GameBootstrap.Initialize/Shutdown.
- ManaManager integrado ao SaveManager capture/apply.
- Tecla E com prioridade para `IInteractable`; se houver interactable próximo, não atacar.
- Active slots R/T/Y/G executam SkillActionSO real, não placeholders.
- ArcaneBolt asset criado via Editor script idempotente.
- Weapon assets reais mínimos além de unarmed.
- Bow/ranged projectile simples com spawn, timeout, colisão e dano.
- Save/load de mana.
- Save/load de active slots por SkillActionId.
- UI mana bar e active skill slots mínima ou interface provisória documentada para SPEC 17.
- Play Mode: Q/E/Space/R/T/Y/G, mana, cooldown, dano, dodge, bow, spell.
...
- Mana inicializa, regenera, gasta e salva/carrega.
- E interage antes de atacar.
- Q/E/Space/R/T/Y/G estão funcionais conforme spec.
- SkillActionSO é executável por slot ativo.
- Bow e ArcaneBolt causam dano via DamageCalculator.
- Spec 13/14/16 ficam desbloqueadas com evidência.
...
```text
PLAY MODE TEST: SPEC 12 — Player Combat, Weapons, Spells e Skill Actions
Scene used:
Steps executed:
Expected result:
Observed result:
...
```bash
git add <arquivos>
git commit -m "feat: finalizar spec 12 - player combat spells skill actions"
```
