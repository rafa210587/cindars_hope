...
# SPEC 16 — Skill Trees, Active Slots e Respec Anya
...
git checkout dev
git pull
git checkout -b feature/spec-16-skill-trees-active-slots-respec
```
...
```text
docs/specs/a_implementar/spec_skill_trees_active_slots_respec_anya_runtime.md
```

## Dependências diretas

Depende de 15. Bloqueia 17.

## Objetivo desta execução

Finalizar progressão de skill trees com 5 árvores, nodes, passivas, skill actions equipáveis, capstones, respec na Fonte de Anya e save/load.
...
- 5 skill trees completas: Melee, Ranged, Magic, Survival, Crafting.
- Skills passivas aplicam efeitos reais nos derived stats/sistemas.
- Skills equipáveis para R/T/Y/G usam SkillActionId.
- Capstones por árvore.
- Prerequisites/dependencies entre nodes.
- Skill tree modal na tecla K ou UI mínima funcional, com polish final para SPEC 17 se necessário.
- Node purchase lógica com SkillPoint a cada 2 níveis.
- Active slot assignment por SkillActionId.
- Respec full na Fonte de Anya.
- Derived stats recalculam ao comprar/respec/load.
- Save/load de nodes comprados + active slots.
- Play Mode: buy node -> acquire skill -> equip slot -> use -> respec -> reload.
...
- SkillPoints são consumidos e restituídos no respec.
- Nodes respeitam prerequisites.
- Passivas afetam runtime real.
- Active skills executam via SPEC 12.
- Save/load preserva árvore e slots.
...
```text
PLAY MODE TEST: SPEC 16 — Skill Trees, Active Slots e Respec Anya
Scene used:
Steps executed:
Expected result:
Observed result:
...
```bash
git add <arquivos>
git commit -m "feat: finalizar spec 16 - skill trees active slots respec"
```
