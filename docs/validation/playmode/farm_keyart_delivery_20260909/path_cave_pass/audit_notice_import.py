...
root=out.parents[4]
asset=root/'Assets/_Game/Art/Generated/World/props/notice_board_keyart_v1.png'
raw=root/'art/farm-pixelart-review/raw/notice-board-keyart-v1.png'
...
result=dict(asset=str(asset.relative_to(root)).replace('\\','/'),raw=str(raw.relative_to(root)).replace('\\','/'),assetSha256=sha(asset),rawSha256=sha(raw),byteExact=sha(asset)==sha(raw),metaSha256=sha(Path(str(asset)+'.meta')),dimensions=[im.width,im.height],mode=im.mode,alphaExtrema=im.getchannel('A').getextrema(),filterMode=integer('filterMode'),enableMipMap=integer('enableMipMap'),pixelsPerUnit=integer('spritePixelsToUnits'),serializedLegacyRootMaxTextureSize=integer('maxTextureSize'),platforms=platforms,scope='Serialized importer + raw PNG verification. DefaultTexturePlatform max2048, adequate for source1254 square; runtime imported texture width was not separately queried.')
result['pass']=result['byteExact'] and result['filterMode']==0 and result['enableMipMap']==0 and result['pixelsPerUnit']==128 and default['maxTextureSize']==2048 and default['compression']==0 and all(p['overridden']==0 for p in platforms) and result['alphaExtrema']==(0,255)
(out/'notice-board-import-review.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
