...
$inputPaths = @(Get-ChildItem (Join-Path $projectRoot 'Assets/_Game/Scripts') -Filter *.cs -Recurse -File | Where-Object { $_.Name -match 'Farm|WorldTilemapGround|WorldSpriteLibrary|RoofRevealController|TreeNode|CameraTransparencySort2D' })
...
Copy-Item -LiteralPath $scene -Destination (Join-Path $out 'FarmScene.unity.snapshot')
Copy-Item -LiteralPath ($scene + '.meta') -Destination (Join-Path $out 'FarmScene.unity.meta.snapshot')
if ($AsyncPlayMode) {
