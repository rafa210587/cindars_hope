...
def inventory(path):
    ...
    trees={}; exterior=[]; gardens=[]; landmarks=[]
    for i,(t,b) in blocks.items():
        ...
        if 'Visual_PathGarden' in path:
            gardens.append(dict(path=path,componentType=t,treeNode='_treeIndex:' in b,position=value(tr,'m_LocalPosition'),scale=value(tr,'m_LocalScale')))
        if any(n in path for n in ('CaveEntrance','Spawn_farm_from_cave','Board_Zrix')):
            landmarks.append(dict(path=path,componentType=t,position=value(tr,'m_LocalPosition'),scale=value(tr,'m_LocalScale'),size=value(b,'m_Size'),offset=value(b,'m_Offset'),isTrigger=value(b,'m_IsTrigger'),targetSpawn=value(b,'_targetSpawnId'),spawnId=value(b,'_spawnId'),gate=value(b,'_transitionGateId'),targetScene=value(b,'_targetCaveScene')))
        if 'FarmPerimeterVisuals' in path:
            exterior.append(dict(path=path,componentType=t,treeNode='_treeIndex:' in b))
    return dict(trees=trees,exterior=exterior,gardens=gardens,landmarks=landmarks)
...
result['beforeLandmarks']=before['landmarks']
result['afterLandmarks']=after['landmarks']
result['gardenObjectCount']=len(set(g['path'] for g in after['gardens']))
result['gardenObjects']=[g for g in after['gardens'] if g['componentType']==4]
result['gardenComponentTypes']=sorted(set(g['componentType'] for g in after['gardens']))
result['gardenColliderCount']=sum(g['componentType'] in (58,60,61,66,68,70,197,256) for g in after['gardens'])
result['gardenTreeNodeCount']=sum(g['treeNode'] for g in after['gardens'])
(out/'scene-identity-review.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
print(json.dumps({k:v for k,v in result.items() if k not in ('changes','beforeLandmarks','afterLandmarks')},indent=2))
