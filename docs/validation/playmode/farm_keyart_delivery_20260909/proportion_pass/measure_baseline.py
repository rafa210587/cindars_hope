...
for i,(t,b) in blocks.items():
    ...
    if not re.search(r'Bridge_01|FarmHouse/(Roof|Door|Wall_)|Coop_01|Barn_01|Station_CheesePress|Station_WineBarrel|Greenhouse|CraftingStation_|Visual_CentralWell|FonteAnya|ShippingBin_|^Bounds/',path): continue
    pos,scale=world(ti)
    if t==61:
    ...
    measurements.append(dict(path=path,sprite=str(png.relative_to(backup)).replace('\\','/'),spriteGuid=g[1],position=pos,lossyScale=scale,fullRectWorldSize=full,alpha05WorldSize=visible,heightRelativeToPlayer=visible[1]/1.1875,note='Serialized single-sprite estimate; runtime metadata is authoritative.'))
