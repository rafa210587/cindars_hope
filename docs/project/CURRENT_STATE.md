...
- WAVE_INTEGRATION_12: BUILD_VALIDATED_SCENE_WIRED_PENDING_HUMAN_PLAYMODE (2026-06-09) — TownScene selected as NPC/dialogue/shop target; reused existing `NpcDataSO`, `NpcController`, `NpcShopController`, `DialogueModal`, `ShopMenuModal`, `BuyPanel`, `SellPanel`, `ShopManager`, `InventoryManager`, `PlayerManager`; `NpcController` now routes DialogueChoice to NextNodeId/CloseDialogue; Pip and Vaalara Wanderer dialogue trees expanded to 10 runtime nodes; merchant shops mapped to `shop_seeds_tools` and `shop_weapons_armor` with real buy/sell API; no scene YAML changes; canonical roster, dialogue sets, movement schedules, shop services, authoring model, human wiring instructions, execution report, and checklist created; human must execute WAVE11+WAVE12 Play Mode before ACCEPTED/next wave continuation
...
*Last updated: 2026-06-09 (WAVE_INTEGRATION_12_NPC_DIALOGUE_SHOP_BRIDGE: TownScene NPC/dialogue/shop bridge reused and strengthened; both assemblies BUILD_VALIDATED; no scene YAML changes; WAVE11/WAVE12 human Play Mode still pending; next wave remains blocked)*
*Next update: after Play Mode checklists 06A-12 pass*
