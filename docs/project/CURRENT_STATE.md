...
## 2026-07-03 — LOTE CODEX_CONVERGENCE (specs codex_01-13) — BUILD_VALIDATED, commitado em dev

- Duas auditorias externas (Codex) foram verificadas por agentes read-only e viraram 13 specs executadas em loop (spec-implementer + verificação do orquestrador + commit por spec). Todas em `a_implementar/` (não promovidas; Play Mode pendente onde aplicável).
- Corrigido (comportamentos permissivos): validador de arquitetura não retorna mais PASS vazio (NOT_CONFIGURED + 4 IProjectValidator registrados); CombatCondition de quest avalia HP real ou falha explícita (log one-shot nos tipos future); farm arar/regar com stamina real, dia real e exigência de ferramenta (temporarySliceMode desligado); nó de minério destravado entrega item_material_copper_ore; skill field_patch não rega mais plantação (efeito real realocado p/ irrigador_portatil; ledger de dívida das 5 feedback-only).
- Robustez: seeds da cave com FNV-1a estável (substitui string.GetHashCode — cave-stable-run); save com escrita atômica (File.Replace + backup + recuperação no load); debug tooling atrás de UNITY_EDITOR||DEVELOPMENT_BUILD.
- Limpeza: 25 classes mortas deletadas (lote 1: 11; lote 2: 14, verificação nome+GUID+auto-bootstrap) + 4 eventos mortos; RewardTableDefinition preservado (RewardGrantResult em uso — desvio documentado).
- Fundação: smoke EditMode de boot-wiring (GameBootstrap reflection); 7 physics layers via RunStep idempotente + GameplayLayerNames (fallback por nome) + ContactFilter2D sem alocação nas queries de combate + obstacle mask do EnemyBrain wireado (avoidance deixa de ser morto).
- Decisões: ADRs 0020-0030 (proposed) — asmdef, LFS de YAML, composition root, floresta TownScene, IMGUI→Canvas, Resources→refs, RNG por domínio, TimeScaleCoordinator, cena async, contrato PPU/Y-sort (emenda ao ADR-0011), pooling/Tilemap da cave. Aguardam decisão humana.
